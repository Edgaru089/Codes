/*
 ...
 SIGNATURE CODE_20181011_LUOGU3384_
 COMMENT 【模板】树链剖分（再写一遍）
*/

#include "Overall.hpp"

// Check if this code file is enabled for testing
#ifdef CODE_20181011_LUOGU3384_
...
namespace segtree {
	...
	void pushdown(node* p) {
		if (p&&p->lazy) {
			p->sum = (p->sum + (p->right - p->left + 1)*p->lazy) % ::p;
			if (p->lson)
				p->lson->lazy = (p->lson->lazy + p->lazy) % ::p;
			if (p->rson)
				p->rson->lazy = (p->rson->lazy + p->lazy) % ::p;
			p->lazy = 0;
		}
	}
...
int querychain(int u, int v) {
	...
	while (top[u] != top[v]) {
		if (dep[top[u]] > dep[top[v]]) {
			sum = (sum + segtree::query(dfn[top[u]], dfn[u])) % ::p;
			u = father[top[u]];
		} else {
			sum = (sum + segtree::query(dfn[top[v]], dfn[v])) % ::p;
			v = father[top[v]];
		}
	}
	sum = (sum + segtree::query(min(dfn[u], dfn[v]), max(dfn[u], dfn[v]))) % ::p;
	return sum;
}
