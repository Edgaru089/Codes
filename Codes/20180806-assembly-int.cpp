/*
DOCUMENT NAME "20180806-assembly-int.cpp"
CREATION DATE 2018-08-06
SIGNATURE CODE_20180806_ASSEMBLY_INT
COMMENT 汇编语言解释器（assembly language interpreter）
*/

/* Ktx-65 Assembly Language Interpreter */
/* Ktx-65 汇编语言解释器 */
/* Copyright (c) Kawasiro Nitori of Kappa Heavy Industry Co.,Ltd. */
/* 版权所有© 河童重工业会社 河城荷取 */
...
#ifdef CODE_20180806_ASSEMBLY_INT
...
(!!(statement) || [](int l, string state, string m)->bool {compileError(l, "Compile Assertation Failed: "s + m + ", Statement: "s + state); return false; }(line, #statement, message))
...
public:
	...
	enum Command {
		...
		NoOp,           // Does nothing
		// Memory Access
		//Push,           // Push <0> into sVal
		//Top,            // Put top of sVal into <0>
		//Pop,            // Pop sVal and put into <0> if present
		//Duplicate,      // sVal.push(sVal.top())
...
private:
	...
	//void push(int val) { if (sVal.size() >= stackSize)runtimeError("sVal Stack Overflow"); sVal.push(val); }
	//int top() { if (sVal.size() <= 0) { runtimeError("sVal Access Error(size = 0)"); return 0; } else return sVal.top(); }
	//void pop() { if (sVal.size() <= 0)runtimeError("sVal Stack Underflow"); else sVal.pop(); }
	//int popRet() { if (sVal.size() <= 0) { runtimeError("sVal Stack Underflow"); return 0; } else { int val = sVal.top(); sVal.pop(); return val; } }
...
private:
	// Command Handlers
	//void funcPush(vector<DataSource>& l) { push(l[0]()); nextptr++; }
	//void funcPop(vector<DataSource>& l) { if (l.size() > 0)l[0]() = popRet(); else pop(); nextptr++; }
	//void funcTop(vector<DataSource>& l) { l[0]() = sVal.top(); nextptr++; }
	//void funcDup(vector<DataSource>& l) { push(top()); nextptr++; }
...
commands[name] = command; names[command] = name
		...
		//REGISTER_HANDLER(Push, funcPush, "PUSH");
		//REGISTER_HANDLER(Top, funcTop, "TOP");
		//REGISTER_HANDLER(Pop, funcPop, "POP");
		//REGISTER_HANDLER(Duplicate, funcDup, "DUP");
					...
					if (cur.word == "%") {
						...
						if (command == "callfunc") {
							...
							if (it == funcStartLines.end())
								compileError(func.line, "Function \"" + func.line + "\" undefined"s);
							else {
...
int main(int argc, char* argv[]) {
	...
	tokens.parse(fin); fin.close();
	compiler.compile(tokens);
	compiler.generateCode(prog);
	if (prog.valid) {
		cout << "Done." << endl;
		prog.run();
		cout << "\nProgram exited after " << prog.getClock() << " clock cycles" << endl;
	}
